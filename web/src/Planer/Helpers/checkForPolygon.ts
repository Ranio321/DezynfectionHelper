...
function getAllIntersectPoints(planerItems: Item[]) {
  ...
  for (let x = 0; x < walls.length; x++) {
    ...
    if (walls.length > 1) {
      let firstLine = {
        start: walls[x].position.start!,
        end: walls[x].position.end!,
      };
      for (let y = 0; y < walls.length; y++) {
        if (x !== y) {
          let secondLine = {
            start: walls[y].position.start!,
            end: walls[y].position.end!,
          };
...
function p1(
  mainId: number,
  prevId: number,
  item: WallsIntersectPoint,
  allPoints: WallsIntersectPoint[],
  end: any
): any {
  if (item.intersectPoints.length > 1) {
    ...
    for (let y = 0; y < item.intersectPoints.length; y++) {
      let newItem = allPoints.find(
        (p) =>
          p.id === item.intersectPoints[y].id &&
          item.intersectPoints[y].id !== prevId
      );
      ...
      if (
        newItem?.intersectPoints.length! > 1 &&
        newItem &&
        newItem.intersectPoints &&
        newItem.intersectPoints.length &&
        newItem?.id !== mainId
      ) {
        end.push(item.intersectPoints[y]);
